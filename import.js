...
let findIndex = () => new Promise((resolve, reject) => {
    let getOptions = {
        headers: {
            'Authorization': 'Basic ' + options.auth,
            'kbn-xsrf': 'reporting'
...
async function run(host, auth, title, oldIndex, newIndex, filename, pre, post) {
...
program
    .version('1.2.0')
    .description('This program imports a dashboard templates into Kibana. See https://github.com/Monkat-Company/kibana-dashboard-migrator for more details.')
    ...
    .on('--help', () => {
        ...
        console.log()
        console.log(
            '    $ import.js -c config/import-int.json'
        )
        console.log()
    })
    .parse(process.argv)
...
prompt.get(schema, function (err, result) {
    ...
    async.eachSeries(importJson['dashboards'], async function (eachitem, next) {
        await run(importJson['host'], auth, eachitem['name'], importJson['oldIndex'], importJson['newIndex'], eachitem['template'], importJson['findInName'], importJson['replaceInName'])
        next();
    }, function () {
        process.exit()
    });
